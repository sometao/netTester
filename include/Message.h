...
class Message {
protected:
  void writeHead(uint8_t* buf, size_t size) {
    if (size < 15) {
 ...
 public:
  ...
  virtual void getBinary(uint8_t* buf, size_t size) = 0;
...
class TestRequest : public Message {
 public:
  ...
  size_t getLength() override { return 23; }

  void getBinary(uint8_t* buf, size_t size) override {
    if (size < getLength()) {
